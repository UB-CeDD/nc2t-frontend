...
const AdminPage: React.FC = () => {
  ...
  const { species, searchResults } = useSelector(
    ...
    searchResults: {
      [x: string]: number;
      results: SearchedSpecies[];
      message?: string;
    };
  };

  const { compounds } = useSelector(
    (state: RootState) => state.getCompounds,
  ) as {
  ...
  const { locations } = useSelector(
    (state: RootState) => state.getLocations,
  ) as {
  ...
  const { herbariums } = useSelector(
    (state: RootState) => state.getHerbariums,
  ) as {
  ...
  const getSpeciesByKingdom = () => {
    if (!species) return [];
    const data: { [key: string]: number } = species.reduce(
      (acc: { [key: string]: number }, s: Species) => {
  ...
  const getCompoundsByClass = () => {
    if (!compounds) return [];
    const data: { [key: string]: number } = compounds.reduce(
      (acc: { [key: string]: number }, c: Compound) => {
  ...
  const getUsersByRole = () => {
    if (!users) return [];
    const data: { [key: string]: number } = users.reduce(
      (acc: { [key: string]: number }, u: UserModel) => {
  ...
  const getReferencesByType = () => {
    ...
    const data: { [key: string]: number } = references.reduce(
      (acc: { [key: string]: number }, r: Reference) => {
  ...
  const handleEdit = async (row: Species | SearchedSpecies | Reference) => {
    if ("kingdom" in row) {
      ...
      if (existingSpecies) {
        navigate(`/dashboard/species/${existingSpecies.id}/edit`, {
          state: { specie: existingSpecies },
        });
      } else {
        navigate(`/dashboard/species/add`, {
          state: { specie: { reference: row } },
        });
      }
    }
  };
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                    {searchResults.results.map(
                      (result: SearchedSpecies | Reference, index: number) => {
                        const specie: SearchedSpecies =
                          "kingdom" in result
                            ? result
                            : {
                                ...(result as Reference),
