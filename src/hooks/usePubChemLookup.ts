...
interface PubChemLookupResult {
  ...
  lookupCompound: (
    compoundName: string,
  ) => Promise<{ cid: number | undefined; smiles: string | undefined }>;
}
  ...
  const lookupCompound = useCallback(async (compoundName: string) => {
    ...
    try {
      ...
      const smilesResponse = await fetch(
        `https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/${fetchedCid}/property/CanonicalSMILES/JSON`,
      );
      ...
      if (
        ...
        !smilesData.PropertyTable.Properties[0].ConnectivitySMILES
      ) {
        try {
          const OpenBabel = await openBabelPromise;
          const sdfResponse = await fetch(
            `https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/${fetchedCid}/SDF`,
          );
          ...
          console.log("Generated SMILES with Open Babel:", {
            cid: fetchedCid,
            smiles: fetchedSmiles,
          });
        } catch (openBabelError) {
          console.error("Open Babel generation failed:", openBabelError);
          setError(
            "PubChem SMILES not found, and Open Babel generation failed.",
          );
          setLoading(false);
        ...
        fetchedSmiles =
          smilesData.PropertyTable.Properties[0].ConnectivitySMILES;
        console.log("Fetched from PubChem:", {
          cid: fetchedCid,
          smiles: fetchedSmiles,
        });
      }
