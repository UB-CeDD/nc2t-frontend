...
const speciesUserRoleReducer = (state = initialState, action: any): SpeciesUserRoleState => {
    switch (action.type) {
        case FETCH_SPECIES_USERS_REQUEST:
            return { ...state, loading: true, error: null };
        case FETCH_SPECIES_USERS_SUCCESS:
            return { ...state, loading: false, speciesUsers: action.payload, error: null };
        case FETCH_SPECIES_USERS_FAILURE:
            return { ...state, loading: false, error: action.payload };
        case ADD_SPECIES_USER_SUCCESS:
            return { ...state, loading: false, speciesUsers: [...state.speciesUsers, action.payload], error: null };
        ...
        case REMOVE_SPECIES_USER_SUCCESS:
            return {
                ...state,
                loading: false,
                speciesUsers: state.speciesUsers.filter((su) => su.user.id !== action.payload),
                error: null,
            };
