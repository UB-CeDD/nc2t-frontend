...
const herbariumReducer = (state = initialState, action: any): HerbariumState => {
    switch (action.type) {
        case FETCH_HERBARIUMS_REQUEST:
            return { ...state, loading: true, error: null };
        case FETCH_HERBARIUMS_SUCCESS:
            return { ...state, loading: false, herbariums: action.payload, error: null };
        case FETCH_HERBARIUMS_FAILURE:
            return { ...state, loading: false, error: action.payload };
        case CREATE_HERBARIUM_SUCCESS:
            return { ...state, loading: false, herbariums: [...state.herbariums, action.payload], error: null };
        case CREATE_HERBARIUM_FAILURE:
            return { ...state, loading: false, error: action.payload };
        case RETRIEVE_HERBARIUM_SUCCESS:
            return { ...state, loading: false, herbarium: action.payload, error: null };
