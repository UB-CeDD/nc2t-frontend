...
import {
  createCompoundThunk,
  updateCompoundThunk,
} from "@store/thunks/compoundThunk.ts";
...
const CompoundForm: React.FC<CompoundProps> = ({
  compound,
  onSave,
  onCancel,
  onCompoundCreated,
  ...
  const { loading: compoundLoading } = useSelector(
  ...
  const handlePubChemButtonClick = async () => {
    if (!formData.name) {
      addNotification(
        t("compound.notifications.enter_name_for_pubchem"),
        "warning",
      );
      return;
    }
    ...
    if (pubchemError) {
      ...
      addNotification(
        t("compound.notifications.pubchem_data_fetched"),
        "success",
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (compound) {
        await dispatch(
          updateCompoundThunk(compound.id, formData, addNotification),
        );
      } else {
        const resultAction = await dispatch(
          createCompoundThunk(formData, addNotification),
        );
        ...
        <form onSubmit={handleSubmit} className="p-4">
            ...
            <div className="flex">
              ...
              >
                {pubchemLoading ? (
                  ...
                  t("compound.form_fields.lookup_pubchem")
                )}
              </button>
            </div>
          </div>
          <div className="mb-6">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, other_names_input: e.target.value })
              }
              onKeyDown={(e) => {
                if (
                  (e.key === "Enter" || e.key === ",") &&
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      setFormData({
                        ...formData,
                        other_names: formData.other_names.filter(
                          (_, i) => i !== idx,
                        ),
                      });
                    }}
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("compound.form_fields.sub_class")}
            </label>
            <input
              required
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("compound.form_fields.pubchem_id")}
            </label>
            <input
              type="number"
              name="pubchem_id"
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("compound.form_fields.bio_activity")}
            </label>
            <input
              type="text"
              name="bio_activity"
