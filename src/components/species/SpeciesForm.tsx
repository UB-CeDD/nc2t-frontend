...
const SpeciesForm: React.FC<SpeciesFormProps> = ({
    ...
    if (initial.compounds && initial.compounds.length > 0) {
      if (typeof initial.compounds[0] === "number") {
        const fetchSelectedCompounds = async () => {
          ...
          const fetchedCompounds = allCompounds.filter((c) =>
    ...
    if (initial.references && initial.references.length > 0) {
      if (typeof initial.references[0] === "number") {
        const fetchSelectedReferences = async () => {
          ...
          const fetchedReferences = allReferences.filter((r) =>
    ...
    if (initial.harvest_sites && initial.harvest_sites.length > 0) {
      if (typeof initial.harvest_sites[0] === "number") {
        const fetchSelectedSites = async () => {
          ...
          const fetchedSites = allLocations.filter((l) =>
            siteIds.includes(l.id),
          );
    ...
    if (initial.storage_locations && initial.storage_locations.length > 0) {
      if (typeof initial.storage_locations[0] === "number") {
        const fetchSelectedStorageLocations = async () => {
          ...
          const fetchedStorageLocations = allLocations.filter((h) =>
            storageLocationIds.includes(h.id),
          );
  ...
  const handleSearchCompound = async (
    ...
    try {
      ...
      const filtered = allCompounds.filter(
        (compound) =>
  ...
  const handleRemoveSelectedHerbarium = (herbariumId: number) => {
    setSelectedStorageLocations((prev) =>
      prev.filter((h) => h.id !== herbariumId),
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.id) {
        ...
        result = dispatch(
          createSpeciesThunk(finalFormData as Species, addNotification),
        );
      }
  ...
  return (
    <div className="flex items-center justify-center">
        ...
        <form onSubmit={handleSubmit} className="p-4">
          <div className="mb-6 flex flex-col">
            ...
            >
              ...
              {["Plantae", "Animalia", "Protista", "Fungi", "Monera"].map(
                (item, index) => (
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("species.form_fields.recent_name")}
            </label>
            <input
              id="recent_name"
              type="text"
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("species.form_fields.trad_uses")}
            </label>
            <input
              id="trad_uses"
              type="text"
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("species.form_fields.part_used")}
            </label>
            <select
              required
              id="part_used"
              ...
              <div className="mt-0 border border-gray-300 rounded-lg p-2 max-h-40 overflow-y-auto">
                {availableReferences.map((reference) => (
                  <div
                    ...
                    onClick={() => handleSelectExistingReference(reference)}
                  >
                    <span>
            ...
            <div className="mt-2">
              {selectedReferences.map((reference) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => handleRemoveSelectedReference(reference.id)}
                    className="flex-shrink-0 ml-1.5 inline-flex text-blue-400 hover:text-blue-500 focus:outline-none focus:text-blue-500"
                  >
                    <svg
                      className="h-2 w-2"
                      stroke="currentColor"
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("species.form_fields.compound_codes")}
            </label>
            <div className="flex items-center gap-2">
              <input
                type="text"
              ...
              <div className="mt-2 border border-gray-300 rounded-lg p-2 max-h-40 overflow-y-auto">
                {availableCompounds.map((compound) => (
                  <div
                    key={compound.id}
                    className="flex justify-between items-center p-1 hover:bg-gray-100 cursor-pointer"
                    onClick={() => handleSelectExistingCompound(compound)}
                  >
                    <span>
                      {compound.smiles} ({compound.compound_class})
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-2">
              {selectedCompounds.map((compound) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => handleRemoveSelectedCompound(compound.id)}
                    className="flex-shrink-0 ml-1.5 inline-flex text-blue-400 hover:text-blue-500 focus:outline-none focus:text-blue-500"
                  >
                    <svg
                      className="h-2 w-2"
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("species.form_fields.collection_date")}
            </label>
            <input
              id="collection_date"
              type="date"
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("species.form_fields.herbarium")}
            </label>
            <div className="flex items-center gap-2">
              <input
                type="text"
              ...
              <div className="mt-2 border border-gray-300 rounded-lg p-2 max-h-40 overflow-y-auto">
                {availableHerbaria.map((herbarium) => (
                  <div
                    key={herbarium.id}
                    className="flex justify-between items-center p-1 hover:bg-gray-100 cursor-pointer"
                    onClick={() => handleSelectExistingHerbarium(herbarium)}
                  >
                    <span>
                      {herbarium.name} ({herbarium.city_town},{" "}
                      {herbarium.country})
                    </span>
            ...
            <div className="mt-2">
              {selectedStorageLocations.map((herbarium) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => handleRemoveSelectedHerbarium(herbarium.id)}
                    className="flex-shrink-0 ml-1.5 inline-flex text-blue-400 hover:text-blue-500 focus:outline-none focus:text-blue-500"
                  >
                    <svg
                      className="h-2 w-2"
      ...
      <Modal
        show={showHerbariumModal}
        onClose={() => setShowHerbariumModal(false)}
      >
