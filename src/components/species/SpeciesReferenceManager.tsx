...
const SpeciesReferenceManager: React.FC<SpeciesReferenceManagerProps> = ({ initialReferences, onSave }) => {
    const dispatch = useDispatch();
    const { references, loading, error } = useSelector((state: RootState) => state.getReferences);
    ...
    useEffect(() => {
        if (initialReferences) {
            setSelectedReferenceIds(initialReferences.map(r => r.id!).filter(id => id !== undefined));
        }
    }, [initialReferences]);
    ...
    return (
        <div className="p-4">
            ...
            <div className="mt-4 flex justify-end">
                ...
                <button
                    onClick={() => onSave(initialReferences.map(r => r.id!).filter(id => id !== undefined))}
