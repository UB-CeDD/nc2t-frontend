...
const SpeciesList: React.FC<SpeciesListProps> = ({ onEditSpecies }) => {
    ...
    { key: "part_used", label: t("species.table_columns.part_used") },
    {
      ...
      render: (row: Species) =>
        Array.isArray(row.references)
          ? (row.references as Reference[]).length
          : 0,
    },
    {
      key: "compounds",
      ...
      render: (row: Species) =>
        Array.isArray(row.compounds) ? (row.compounds as Compound[]).length : 0,
    },
  ];

  const handleEdit = async (row: Species | SearchedSpecies | Reference) => {
    if ("kingdom" in row) {
      // It's a Specie or SearchedSpecie
      onEditSpecies(row);
    } else {
      // It's a Reference that might not have a specie yet
  ...
  const errorMessageText = displayErrorMessage
    ? typeof displayErrorMessage === "object"
      ? JSON.stringify(displayErrorMessage)
      : displayErrorMessage
    : "";
  ...
  return (
    <div className="flex items-center justify-center">
      <div
        className={`bg-${searchResults !== null && searchTerm.trim() !== "" ? "gray" : "white rounded shadow-md w-full p-8"} `}
      >
        {errorMessageText && (
          <div
