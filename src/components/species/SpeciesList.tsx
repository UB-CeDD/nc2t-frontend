...
import { fetchSpecies, deleteSpeciesThunk } from '@store/thunks/speciesThunk.ts';
...
const SpeciesList: React.FC<SpeciesListProps> = ({ onEditSpecies }) => {
    ...
    const [speciesForDelete, setSpeciesForDelete] = useState<Species | null>(null);
    ...
    const handleDelete = (row: Species) => {
        setSpeciesForDelete(row);
        setShowDeleteModal(true);
    };

    const handleConfirmDelete = async () => {
        if (!speciesForDelete) return;
        
        setIsDeleting(true);
        try {
            await dispatch(deleteSpeciesThunk(speciesForDelete.id.toString(), addNotification));
            setShowDeleteModal(false);
            setSpeciesForDelete(null);
        } catch (error) {
    ...
    return (
        <>
            ...
            <DeleteConfirmationModal
                ...
                itemName={speciesForDelete?.name || ''}
                moduleName={t('species.module')}
