...
const SpeciesHerbariumManager: React.FC<SpeciesHerbariumManagerProps> = ({
  initialHerbariums,
  onSave,
}) => {
  const dispatch = useDispatch();
  const { herbariums, loading, error } = useSelector(
  ...
  useEffect(() => {
    if (initialHerbariums) {
      setSelectedHerbariumIds(
        initialHerbariums.map((h) => h.id!).filter((id) => id !== undefined),
      );
    }
  }, [initialHerbariums]);
          ...
          herbariums.map((herbarium) => (
            <div key={herbarium.id} className="flex items-center mb-2">
              ...
              <label htmlFor={`herbarium-${herbarium.id}`}>
                {herbarium.location.name} ({herbarium.location.city_town},{" "}
                {herbarium.location.country})
              </label>
            </div>
          ))
        )}
