...
const SpeciesCompoundManager: React.FC<SpeciesCompoundManagerProps> = ({
  initialCompounds,
  onSave,
}) => {
  const dispatch = useDispatch<AppDispatch>();
  const { compounds, loading, error } = useSelector(
  ...
  useEffect(() => {
    if (initialCompounds) {
      setSelectedCompoundIds(
        initialCompounds.map((c) => c.id!).filter((id) => id !== undefined),
      );
    }
  }, [initialCompounds]);
