...
import {
  addSpeciesUserThunk,
  fetchSpeciesUsersThunk,
  removeSpeciesUserThunk,
  updateSpeciesUserRoleThunk,
} from "@/store/thunks/speciesUserRoleThunk";
...
const SpeciesUserManagement: React.FC = () => {
  ...
  const { speciesUsers, loading, error } = useSelector(
    (state: RootState) => state.speciesUserRoles,
  );
  ...
  const handleAddUser = () => {
    if (speciesId && selectedUser && selectedRole) {
      dispatch(
        addSpeciesUserThunk(speciesId, parseInt(selectedUser), selectedRole),
      );
      setSelectedUser("");
    }
  };
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      handleRoleChange(
                        user.user.id!,
                        e.target.value as SpeciesUserRole,
                      )
                    }
