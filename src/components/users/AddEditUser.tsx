...
const AddEditUser: React.FC<AddEditUserProps> = ({ user, onClose }) => {
  ...
  useEffect(() => {
    if (user) {
      // If user prop is provided, use it directly
      setFormData({
        email: user.email || "",
      ...
      // Otherwise, if id is in params, fetch user details
  ...
  useEffect(() => {
    if (userDetails && id && !user) {
      // Only update from fetched userDetails if no user prop and id exists
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (user?.id) {
        // Use user.id if user prop is present
