...
} from "@store/thunks/referenceThunk";
...
const ReferenceForm: React.FC<ReferenceFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (reference?.id) {
        await dispatch(
          updateReferenceThunk(reference.id, formData, addNotification),
        );
      } else {
        const newReference = await dispatch(
          createReferenceThunk(formData, addNotification),
        );
        if (newReference) {
    ...
    <div className="flex items-center justify-center">
      <div className="bg-white p-8 rounded shadow-md w-full">
        ...
        <form onSubmit={handleSubmit} className="p-4">
          ...
          <div className="mb-6">
            ...
            >
              <option value="" disabled>
                {t("reference.form_fields.type_placeholder")}
              </option>
          ...
          <div className="mb-6">
            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
              {t("reference.form_fields.author")}
            </label>
            <input
              required
              type="text"
          ...
          {formData.type === "Thesis" && (
            <div className="mb-6">
              <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-dark">
                {t("reference.form_fields.thesis_level")}
              </label>
              <select
                required
                name="thesis_level"
