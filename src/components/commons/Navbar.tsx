...
const Navbar: React.FC = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (isDropdownOpen && event.target instanceof Element && !event.target.closest('#user-menu-button')) {
    ...
    return (
                ...
                {isDropdownOpen && (
                    <div className="z-50 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
                        ...
                        <ul className="py-2" aria-labelledby="user-menu-button">
                            ...
                            <li>
                                <button onClick={() =>{ handleLogout(); console.log('User logged out');}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Sign out</button>
                            </li>
                        </ul>
                    </div>
                )}
        ...
        </div><div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-user">
